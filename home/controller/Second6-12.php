...
//			  $this->assign('jlzss',$jlzss);
...

            {
                ...
                $gl=$obj->join($join)->where('s.house_id',$fangid)->where('s.model','second_house')->group('s.user_id')->select();
                // print_r($gl);
                $this->assign('gl',$gl);
                $this->assign('ud',$ud);
...

    {
        ...
        if($sort==8){
            ...
            $lists   = $obj->where($where)->where('s.top_time','lt',$time)->where('s.fcstatus','eq',169)->field($field)->order($this->getSort($sort))->paginate(10,false,['query'=>['keyword'=>$keyword]]);
        }else
        {
