...
$field .= ',m.years';
                ...
                $tong = $objss->field($field)->join($joinss)->where('estate_id',$xiaoqu)->where('fcstatus',175)->where('s.status',1)->select();
...

    {
...

        {
            ...
            $lists = $obj->join($join)->where($where)->where('m.model','second_house')->where('s.top_time','lt',$time)->field($field)->group('s.id')->order($this->getSort($sort))->paginate(20,false,['query'=>['keyword'=>$keyword]]);

        }else{
        if($sort==8){
            $lists   = $obj->where($where)->where('s.top_time','lt',$time)->where('s.fcstatus','neq',169)->field($field)->order($this->getSort($sort))->paginate(20,false,['query'=>['keyword'=>$keyword]]);
        }else if($sort==7){
            $lists   = $obj->where($where)->where('s.top_time','lt',$time)->where('s.fcstatus','eq',170)->field($field)->order($this->getSort($sort))->paginate(20,false,['query'=>['keyword'=>$keyword]]);
        }else
        {
