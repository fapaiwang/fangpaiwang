             ...
             if($info['bianhao_hao']==$today){
               ...
               $data['bianhao_hao']=$today;
                ...
                $data['bianhao']='001';
                 ...
                 // $info=$obj->max('id')->find();


             // $bianhao="TJSCGZ".$today;
            
           // $query = "select max(Id) as maxId from info where Id like '$today%'";//在数据库中查询当日是否已经有编号产生，如果有返回最大值的那一条
           // $info  = $obj->where('bianhao','like',$bianhao.'%')->max('bianhao')->find();
             // $info  = $obj->where('bianhao','like',$bianhao.'%')->find();

         // print_r($data['bianhao_hao']);
         //  print_r($data['bianhao']);
         // exit();

            // $data['bianhao']="TJSCGZ001";
