...

{
...

    {
        ...
        $field = 'u.id,u.nick_name,u.description,u.lxtel_zhuan,u.mobile,u.online_consulting,d.company,d.history_complate,d.service_area,d.tags,d.point,(FORMAT((d.point/5)*100,0)) as goods';
...

    {
...

        {
            ...
            $field = 'id,city,title,estate_id,estate_name,fcstatus,img,room,kptime,living_room,toilet,acreage,price,average_price,address,orientations,renovation,tags,marketprice';

            $second_lists = $secondObj->where($where)->field($field)->order('create_time desc')->limit(20)->select();
            ...
            $rental_lists = $rentalObj->where($where)->field($field)->order('create_time desc')->limit(20)->select();
            ...
            $comment_lists = $commentObj->where($where)->field($field)->order('create_time desc')->limit(20)->select();
			...
			$this->assign('secondtotal',$count['second_total']);
...

    {
       ...
       //	$where['broker_id'] = $id;
		...
		$secondObj = model('second_house');
		$field = 'id,city,title,estate_id,estate_name,fcstatus,img,room,kptime,living_room,toilet,acreage,price,average_price,address,orientations,renovation,tags,marketprice';
	   	$second_lists = $secondObj->where($where)->field($field)->order('create_time desc')->limit($start, 20)->select();
        ...
        return (array( 'second_lists'=>$second_lists,'tags'=>getLinkMenuCache(14), 'msg'=>'获取成功！'));
...

    {
...

        {
            ...
            $field = 'id,fcstatus,city,title,estate_id,estate_name,img,room,living_room,toilet,acreage,price,average_price,address,orientations,renovation,tags,marketprice';

            $lists = model('second_house')->where($where)->field($field)->order('create_time desc')->paginate(20);
