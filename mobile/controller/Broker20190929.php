...

{
...

    {
...

        {
            ...
            $field = 'id,city,title,estate_id,estate_name,fcstatus,img,room,living_room,toilet,acreage,price,average_price,address,orientations,renovation,tags,marketprice';

            $second_lists = $secondObj->where($where)->field($field)->order('create_time desc')->limit(5)->select();
...

    {
       ...
       //	$where['broker_id'] = $id;
		...
		$secondObj = model('second_house');
		$field = 'id,city,title,estate_id,estate_name,fcstatus,img,room,living_room,toilet,acreage,price,average_price,address,orientations,renovation,tags,marketprice';
	   	$second_lists = $secondObj->where($where)->field($field)->order('create_time desc')->limit($start, 5)->select();
 		//$list = db('apps')->limit($start, 5)->order('AppID asc')->select();
		
		return $second_lists;
		
        //return (array( 'result'=>$second_lists,'status'=>1, 'msg'=>'获取成功！'));
