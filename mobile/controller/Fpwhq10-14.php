...
//状态：正常 一拍
$yp[]=['fabutimes','egt',$begin_times];
$yp[]=['fabutimes','elt',$end_times];
$yp[] = ['status','eq',1];
$yp[] = ['jieduan','eq',161];
$yipai = model('second_house')->where($yp)->count();

//状态：正常 二拍
$ep[]=['fabutimes','egt',$begin_times];
$ep[]=['fabutimes','elt',$end_times];
$ep[] = ['status','eq',1];
$ep[] = ['jieduan','eq',162];
$erpai = model('second_house')->where($ep)->count();

//状态：正常 二拍变卖
$bm[]=['fabutimes','egt',$begin_times];
$bm[]=['fabutimes','elt',$end_times];
$bm[] = ['status','eq',1];
$bm[] = ['jieduan','eq',163];
...
$rqs = strtotime(date('Y-m-d 23:59:59'));
$jrxz[]=['fabutimes','egt',$rq];
$jrxz[]=['fabutimes','elt',$rqs];
$jrxz[] = ['status','eq',1];
...
$zt1=date("Y-m-d 00:00:00",strtotime("-1 day"));

$zrcj1[] = ['status','eq',1];
$zrcj1[] = ['fcstatus','eq',175];
$chengjiao=model('second_house')->where($zrcj1)->select();
$azrcj=0;
foreach ($chengjiao as $key => $value) {
...
$t1[]=['fabutimes','egt',$aaa];
$t1[]=['fabutimes','egt',$bbb];
$t1[]=['status','eq',1];
$t1[]=['fcstatus','eq',175];
//成交价
...
//新增
$xz[]=['fabutimes','egt',$begin_times];
$xz[]=['fabutimes','elt',$end_times];
$xz[] = ['status','eq',1];
$xinzeng = model('second_house')->where($xz)->count();

//成交
$cj[]=['fabutimes','egt',$begin_times];
$cj[]=['fabutimes','elt',$end_times];
$cj[] = ['status','eq',1];
$cj[] = ['fcstatus','eq',175];
...
//500万以下
$wb[]=['fabutimes','egt',$begin_times];
$wb[]=['fabutimes','elt',$end_times];
$wb[] = ['status','eq',1];
$wb[] = ['qipai','elt',500];
...
//500万-1000万
$wbyq[]=['fabutimes','egt',$begin_times];
$wbyq[]=['fabutimes','elt',$end_times];
$wbyq[] = ['status','eq',1];
$wbyq[] = ['qipai','gt',500];
$wbyq[] = ['qipai','lt',1000];
...
//1000万以上
$yq[]=['fabutimes','egt',$begin_times];
$yq[]=['fabutimes','elt',$end_times];
$yq[] = ['status','eq',1];
$yq[] = ['qipai','egt',1000];
$yqw = model('second_house')->where($yq)->count();


$dq[]=['fabutimes','egt',$begin_times];
$dq[]=['fabutimes','elt',$end_times];
//东城区
...
$b2 = model('second_house')->where(['city'=>$a2['id']])->where($dq)->count();
$this->assign('b2',$b2);
//朝阳区
$a3 = model('city')->where(['name'=>'朝阳区'])->find();
$b3 = model('second_house')->where(['city'=>$a3['id']])->where($dq)->count();
$this->assign('b3',$b3);
//丰台区
$a4 = model('city')->where(['name'=>'丰台区'])->find();
$b4 = model('second_house')->where(['city'=>$a4['id']])->where($dq)->count();
$this->assign('b4',$b4);
//石景山区
$a5 = model('city')->where(['name'=>'石景山区'])->find();
$b5 = model('second_house')->where(['city'=>$a5['id']])->where($dq)->count();
$this->assign('b5',$b5);
//海淀区
$a6 = model('city')->where(['name'=>'海淀区'])->find();
$b6 = model('second_house')->where(['city'=>$a6['id']])->where($dq)->count();
$this->assign('b6',$b6);
//房山区
$a7 = model('city')->where(['name'=>'房山区'])->find();
$b7 = model('second_house')->where(['city'=>$a7['id']])->where($dq)->count();
$this->assign('b7',$b7);
//通州区
$a8 = model('city')->where(['name'=>'通州区'])->find();
$b8 = model('second_house')->where(['city'=>$a8['id']])->where($dq)->count();
$this->assign('b8',$b8);
//顺义区
$a9 = model('city')->where(['name'=>'顺义区'])->find();
$b9 = model('second_house')->where(['city'=>$a9['id']])->where($dq)->count();
$this->assign('b9',$b9);
//昌平区
$a10 = model('city')->where(['name'=>'昌平区'])->find();
$b10 = model('second_house')->where(['city'=>$a10['id']])->where($dq)->count();
$this->assign('b10',$b10);
//大兴区
$a11 = model('city')->where(['name'=>'大兴区'])->find();
$b11 = model('second_house')->where(['city'=>$a11['id']])->where($dq)->count();
$this->assign('b11',$b11);
//怀柔区
...
$b14 = model('second_house')->where(['city'=>$a14['id']])->where($dq)->count();
$this->assign('b14',$b14);
//延庆区
