...

{
...

    {
        ...
        $field = "distinct(h.id),h.title,h.kptime,h.estate_name,h.fcstatus,h.qipai,h.img,h.city,h.room,h.living_room,h.toilet,h.price,h.average_price,h.tags,h.address,h.acreage,h.orientations,h.renovation";
...
// print_r($lists);
        ...
        if(!empty($gzfy)){
            ...
            model('follow')->where(['id'=>$gzfy['id']])->update(['shuliang'=>$gzfys]);
        ...
        $fields = 'h.id,h.title,h.city,h.img,h.house_type,h.years,h.address,h.price,h.complate_num';
        ...
        $fields .= ','.$second_sql.' as second_total,'.$rental_sql.' as rental_total';

        $fields  = str_replace('9999','h.id',$fields);

        $xqlists = $this->getLists('estate',$fields);
        ...
        //     model('follow')->where(['id'=>$gzxq['id']])->update(['shuliang'=>$gzxqs]);
        ...
        $this->assign('xqpages',$lists->render());
