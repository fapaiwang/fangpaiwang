...
if($ye==0){
    ...
    $start=$ye*20+1;
    $end=($ye+1)*20;
    $lists = db('second_house')->where($jiage)->order('fabutimes','desc')->limit($start,$end)->select();
}
// $lists = db('second_house')->where($jiage)->order('qipai','desc')->limit(20)->select();
        
        $shuliang = model('second_house')->where($jiage)->order('fabutimes','desc')->count();
         model('user')->where(['id'=>$info['id']])->update(['tzgg'=>$shuliang]);
